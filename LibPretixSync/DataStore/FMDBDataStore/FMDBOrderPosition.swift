...
extension OrderPosition: FMDBModel {
    ...
    "secret", "subevent", "pseudonymization_id", "answers_json", "seat_id", "seat_name", "seat_guid", "blocked")
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?);
    """
    ...
    static func from(result: FMResultSet) -> OrderPosition? {
        ...
        let orderPosition = OrderPosition(
            ...
            subEvent: subevent, pseudonymizationId: pseudonymization_id, checkins: [], answers: answers, seat: Seat(seat_id, seat_name, seat_guid), requiresAttention: nil, addonTo: addOntoId, blocked: blocked)
    ...
    func adding(checkIns newCheckIns: [CheckIn]) -> OrderPosition {
        return OrderPosition(
            ...
            subEvent: subEvent, pseudonymizationId: pseudonymizationId, checkins: newCheckIns, answers: answers, seat: self.seat, requiresAttention: self.requiresAttention, addonTo: self.addonTo, blocked: self.blocked)
    }
    ...
    func adding(item: Item?) -> OrderPosition {
        return OrderPosition(
            identifier: identifier, orderCode: orderCode, orderStatus: orderStatus, order: order, positionid: positionid, itemIdentifier: itemIdentifier, item: item,
            variation: variation, price: price, attendeeName: attendeeName, attendeeEmail: attendeeEmail, secret: secret,
            subEvent: subEvent, pseudonymizationId: pseudonymizationId, checkins: checkins, answers: answers, seat: self.seat, requiresAttention: self.requiresAttention, addonTo: self.addonTo, blocked: self.blocked)
    }
    ...
    func adding(answers: [Answer]?) -> OrderPosition {
        ...
        return OrderPosition(
            identifier: identifier, orderCode: orderCode, orderStatus: orderStatus, order: order, positionid: positionid, itemIdentifier: itemIdentifier, item: item,
            variation: variation, price: price, attendeeName: attendeeName, attendeeEmail: attendeeEmail, secret: secret,
            subEvent: subEvent, pseudonymizationId: pseudonymizationId, checkins: checkins, answers: mergedAnswers.values.map { $0 }, seat: self.seat, requiresAttention: self.requiresAttention, addonTo: self.addonTo, blocked: self.blocked)
    }
}
